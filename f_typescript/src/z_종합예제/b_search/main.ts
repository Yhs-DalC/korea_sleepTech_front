...
const displayUsers = (users: UsersType) => {
  ...
  if(userList){// userList기 null이 아니라면
...
const sortUsers = (users: UsersType, key: 'name' | 'email'): UsersType => {
  // 지정된 키(name | email)기준으로 사용자 정보 정렬

  //? [...users]
  // : 새로운 바열 주소값에 기존의 사용자 데이터(요소)만 복사해서 가져옴
  // - 깊은 복사
  // - 배열의 불변성을 보완
