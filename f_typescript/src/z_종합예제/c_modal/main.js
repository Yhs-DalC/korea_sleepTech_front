    ...
    var addEventListener_1 = function (users) {
        ...
        if (userList) {
            userList.addEventListener('click', function (e) {
                ...
                // - target: 이벤트가 처음 발생한 DOM요소 (클릭이 일어난 요소)
                // - currentTarget: 발생한 이벤트가 등록된(이벤트 핸들러가 바인딩 된) DOM요소
                var target = e.target;
            ...
            var modal_1 = document.getElementById('user-modal');
            var closeModal = document.querySelector('.close');
            if (modal_1 && closeModal) {
                closeModal.addEventListener('click', function () {
                    modal_1.style.display = 'none';
                });
                // 브라우저 전체를  DOM요소로 반환(브라우저 탬의 전체 영역)
                // document: window에 로드되는 HTML문서 그 자체
