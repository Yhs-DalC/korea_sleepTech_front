...
//! 사용자의 데이터 요청에 대한 '응답을 처리'하는 함수 정의
...
//! 비동기 작업을 사용하여 JSONPlaceholder에서 '사용자 데이터'를 가져오는 함수 정의
async function fechUserData(userId: number) {
  ...
  try{
    ...
    // fetch함수: 해당 경로를 통해 데이터를 가져와 Response 객체를 응답하는 함수
    ...
    handleResponse({error: e instanceof Error ? e.message : 'Umknown Error'});
  }
}
fechUserData(1);
fechUserData(222);// 네트워크 응답이 실패하였습니다
//fechUserData('안녕');