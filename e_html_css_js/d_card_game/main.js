...
//+) 게임의 초기화, 카드 뒤집, 매칭 검사, 게임 완료 확인인 등의 기능을 포함
...
document.addEventListener('DOMContentLoaded',() => {
  ...
  function initializeGame(){
    ...
    //? 카드 컨테이너 내부의 HTML을 비움(초기화)
    // : 게임 새로 시작시 기존 카드를 제거
    cardContainer.innerHTML = '';

    // 12개의 카드를 for반복문으로 생성하여 HTML요소로 할당
    for (let i = 0; i < 12; i++) {
    ...
    //# 시작시 잠시동안 모든 카드의 뒷면(색상)을 공개하는 함수 정의
    function revealCardsTemporary(){
      // 'completed-button' 완료 버튼을 비활성화
      // : 카드가 뒤집힐 당시에 사용자의 컨트롤을 막는 목절

      // DOM요소.속성 = 속성값;
      // cf) DOM요소/setAttribute(속성, 속성값)
      //? bool 속성인 경우 속성값에 boolean값을 가짐
      // : <input type="text" autofocus ?>
      // : inputElement(DOM요소).autofocus = true;

      completedButton.disabled = true;// disabled  속성: 요소에 대한 컨트롤의 비활성화를 가짐
...
function shuffle(array){
  ...
  for (let i = length - 1; i > 0; i--){
    // 현재 요소 i와 무작위로 선택된 요소  j의 위치를 교환
    let j = Math.floor(Math.random() * (i + 1));
    // Math.random()*(i+1): 0부터 1까지의 무작위 인덱스를 생성
    // Math.floor(): 소숫점 아래를 버리고 정수 인덱스를 반환
