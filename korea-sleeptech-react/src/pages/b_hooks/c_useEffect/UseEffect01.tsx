...
function UseEffect01() {
  ...
  // 부수 효과를 나타내는 골백 함수는 필수
  // 의존성 배열의 경우 선택 >> 모든 랜더링 마다 실행
  useEffect(() => {
    console.log(`Count: ${count}`);

    // count 값의 상태가 변경될 때만 부수효과가 실행
  }, [count]);
  ...
  useEffect(() => {
    // 첫 마운트 시에만 실행(모든 상태 변화에 반응하지 않음)
  ...
  return (
    <div>
      ...
      : useEffect의 의존성 배열에 count만 있는 부수 효과는 name의 변경에 반응하지 않는다
      */}
