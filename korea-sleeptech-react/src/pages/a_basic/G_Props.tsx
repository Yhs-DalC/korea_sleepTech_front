...
import { ChildComponent } from './F_Props';
...
export const Wrapper = ({children}: ChildrenType) => {
  return(
    <div style={{border: '2px solid black', padding: '16px', backgroundColor: 'lightgreen'}}>
      {/* props로 전달받은 UI를 HTML내에서 출력시 JS의 문법 출력 처럼 사용 */}
      {children}
    </div>
  )
}
...
function G_Props() {
  ...
  return (
    <div>
    // ReactNode를 props로 전달받는 컴포넌트는 열리고 닫히는 태그 내에서 전달할 ReactNode 작성
    
      {/* 
      ==콘솔창에 입력값 2번 실행==
      strictMode >> import {StrictMode} form 'react'
      : main.tsx에서 명시
      - 렌더링 시 컴포넌트를 일부러 콘솔창에 두 번 실행(부작용 확인)
      */}
      <Wrapper>
        <ChildComponent name='이름'/>
      </Wrapper>
