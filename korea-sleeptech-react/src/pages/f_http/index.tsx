...
function Index() {
  ...
  */
  ...
  const fetchUsers = async () => {
    try {
      //! axios의 메소드 사용 방법(headers 정보는 제외)
      // - axios.get('url')
  ...
  const updateUser = async(id: number, updateUser: User) => {
    try{
      ...
      if(users){
        setUsers(users.map(user => user.id === id ? respose.data : user));
      }
  ...
  return (
    <div>
      ...
      {/* 
      users 데이터가 존재할 경우 UI 출력, 그렇지 않을 경우 p태그(데이터를 가져오는 중입니다)
      */}
      {/* {users?.map(user => (
        <div>
          <h4>{user.name}</h4>
