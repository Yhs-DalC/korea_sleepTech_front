...
// EX) 로그인 상태, 테마 설정(다크/라이트 모드), 다국어 처리(언어 설정), 사용자 정보 등
...
// : Provider는 value속성을 통해 자식 컴포넌트에게 데이터를 전달
...
interface UserContextType {
  ...
  //    : 상태를 업데이트 할 수 있는 값 또는 함수 타입

  // cf) React.Dispatch<T>
  //    : 어떤 값을 받아서 처리하는 함수 타입
  setUser: React.Dispatch<React.SetStateAction<IUser | null>>;
  // >> 값 또는 함수를 받아서 상태를 업데이트하는 함수
}

// Context 생성
// : 사용자 정보를 저장할 context 객체 생성
// - 초기값 undefined 설정 (null체크에 대한 안정성 확보)
...
function Context() {
  return (
    <div>
      ...
      {/* 
      <ExampleNaviBar/>
      context는 provider 내에서 사용되어야 한다
      */}
    </div>
  );
}
